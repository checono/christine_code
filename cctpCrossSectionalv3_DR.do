...
global tableName "QDM6_ServedBeneAnalysis2_TEST_101"	//name of excel file
...
*mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm
global dep "1"
local mlist  ppay_total ppay_snf ppay_hha ppay_car ppay_inp ppay_out ppay_hsp ppay_dme //ppay_inp_index // readmit obs ed ppay_total 
	...
	if ("`m'" == "readmit"){
			...
			*set measure name
			if ("`m'" == "ppay_total"){
				...
				egen total_payment_30 = rowtotal( payment_carrier_30  payment_hha_30 payment_inpatient_index_30 payment_inpatient_other_30 payment_outpatient_30 payment_snf_30) 
			}
			else if ("`m'" == "ppay_out"){
			...
			else if ("`m'" == "ppay_inp_index"){
				global dep  "payment_inpatient_index_30"
			}
...
foreach m of local mlist{
	...
	preserve 
		...
		label(1 "Served Beneficaries") label(2 "Matched Beneficaries") on) ///
		ytitle("`xLab'") xscale(titlegap(2)) ///
		xlabel(	  1 `" "Q1" "(Feb 2012 -" "Apr 2012)" "' ///
				  2 `" "Q2" "(May 2012 -" "Jul 2012)" "' ///
				  3 `" "Q3" "(Aug 2012 -" "Oct 2012)" "' ///
