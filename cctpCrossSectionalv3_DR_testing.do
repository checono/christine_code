...
global date    "160106_101_nohrr_newIP"
...
*set excel workbook name for result ouptu
global tableName "QDM6_ServedBeneAnalysis_newIP_101_nohrr"	//name of excel file
...
*mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm
global dep "1"
local mlist   ppay_total ppay_ip_only ppay_ip_ach ppay_ip_ltch ppay_ip_ipr ppay_ip_ipf ppay_ip_other ppay_out ppay_snf ppay_snf ppay_hha ppay_dme ppay_hsp ppay_car	
local indepVar dual_qtr snf_disch age male black hisp oth_dk charlindex admissions readmissions los ed_visits_ip ed_visits_op  pay*pre1  imp_hcc_4cctp cc* ami hf pnu copd bdtot admtot vem voth for_profit percent_medicare percent_medicaid cmi teach_hospital
// i.hrrnum
	...
	if ("`m'" == "readmit"){
			...
			*set measure name
			if ("`m'" == "ppay_total"){
				...
				egen total_payment_30 = rowtotal(  payment_ip_index_30 payment_ip_non_index_30 payment_carrier_30  payment_hha_30   payment_outpatient_30 payment_snf_30 payment_dme_30 payment_hospice_30) 
			}
			else if("`m'" ==  "ppay_ip_only"){
			    global dep "ppay_ip_only"
				egen ppay_ip_only = rowtotal( payment_ip_other_30 payment_ip_ltch_30 payment_ip_ipr_30 payment_ip_ipf_30 payment_ip_acute_all_30)
			}
			else if("`m'" == "ppay_ip_ach"){
				global dep "ppay_ip_ach"
				egen ppay_ip_ach = rowtotal(payment_ip_acute_all_30)
			}
			else if("`m'" == "ppay_ip_ltch"){
				global dep "ppay_ip_ltch"
				egen ppay_ip_ltch = rowtotal(payment_ip_ltch_30)
			}
			else if("`m'" == "ppay_ip_ipr"){
				global dep "ppay_ip_ipr"
				egen ppay_ip_ipr = rowtotal(payment_ip_ipr_30)
			}
			else if("`m'" == "ppay_ip_ipf"){
				global dep "ppay_ip_ipf"
				egen ppay_ip_ipf = rowtotal(payment_ip_ipf_30)
			}
			else if("`m'" == "ppay_ip_other"){
				global dep "ppay_ip_other"
				egen ppay_ip_other = rowtotal(payment_ip_other_30)
			}
			else if ("`m'" == "ppay_out"){
...
*/
	use "${baseData}\servedBeneAnalyticFile_TEST_2.dta",clear
		sort bene_id clm_id
		capture drop _merge
	...
	* loop through all quarters for cross-sectional regressions
	foreach q of local qtrs{
		preserve 
			...
			*run LPM/OLS model
			reg ${dep} served `indepVar' `qfix', vce(cluster prvdr_num)
