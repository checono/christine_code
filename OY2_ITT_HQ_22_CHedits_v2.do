...
* Use only a 1% sample of this dataset; this 10% sample will be restored each time
sample 1
...
global date "20160404"

log using OY2_ITT_22_CHedits_v2_${date}.txt, replace
...
******************************************
disp "Q reg step"
* Generate pvdr and iqtr dummy variables 
* NOTE: THESE TAB COMMANDS ARE INSIDE THE `w' LOOP WHERE COHORT IS SELECTED (Lines 179:182)
tab pvdr , gen(ipvdr)
tab qtr , gen(iqtr)
...
foreach var of varlist ipvdr*{
	global pvdrInSample "$pvdrInSample `var'"
	}

foreach var of varlist iqtr*{
	global qtrInSample "$qtrInSample `var'"
	}

	
** THIS SECTION IS A SWITCH TO EXCLUDE CREATED PVDR DUMMIES BASED ON THE PVDRS
** THAT ARE OMITTED FROM THE LINEAR REGRESSION B/C OF COLLINEARITY
** You can comment this out as needed for testing 
...
foreach var of varlist ipvdr*{
	...
	if "`w'" == "all"{
		...
		*omit based on pvdrs omitted in linreg
		if "`var'" != "ipvdr1" & "`var'" != ""{
			global pvdrInSample "$pvdrInSample `var'"
		}
	}
		...
		*originally, ipvdr34 corresponds to pvdr37
		if "`var'" != "ipvdr1" & "`var'" != "ipvdr34"{
			global pvdrInSample "$pvdrInSample `var'"
		}
	}
		...
		*ipvdr14 corresponds to pvdr16
		if "`var'" != "ipvdr1" & "`var'" != "ipvdr14"{
			global pvdrInSample "$pvdrInSample `var'"
		}
	}
...
} //close foreach var of varlist ipvdr* loop 

foreach var of varlist iqtr*{
	if "`var'" != "iqtr1" & "`var'" != "iqtr14"{
		global qtrInSample "$qtrInSample `var'"
		}
}//close foreach var of varlist iqtr* loop 
...
disp "Q reg: `w'"
*qreg  mpayinp hbpci post BPCI BPCI_case $covariates ipvdr2-ipvdr52 iqtr2-iqtr13 , q(0.5)  nolog // ORIGINAL 
qreg  mpayinp hbpci post BPCI BPCI_case $covariates $pvdrInSample $qtrInSample , q(0.5) nolog 
